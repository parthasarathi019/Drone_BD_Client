...
const My_Toys = () => {
  ...
  const url = (`http://localhost:9000/mydrones?email=${user?.email}`)
  useEffect(() => {
    fetch(url)
  ...
  return (
    <div>
     <div className='flex justify-center my-1'> <button onClick={()=>SetAsc(!asc)} className='btn btn-primary'>{asc ? "Price high to low" : "Price Low to low"}</button></div>
